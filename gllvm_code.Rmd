...
```{r}
# Read your data
sub_benthos <- fread("~/Project 1/Data Model/sub_benthos_updated.csv")
...
unique(sub_benthos_filtered$species) # and only the 42 species are remining 
```
...
```{r}
#** Bed Shear Stress ----
shear_stress_UTM <- readRDS("~/Project 1/Data Model/shear_stress_UTM.rds")

#** BPI ----
bpi_UTM <- readRDS("~/Project 1/Data Model/bpi_UTM_all.rds")

# mud
mud_UTM <- readRDS("~/Project 1/Data Model/mud_model.rds")
```
...
grid <- readRDS("~/Project 1/Data Model/mud_pred_pit.rds") # Prediction sand pit sites
...
The gllvm package requires data in a site × species matrix format:
